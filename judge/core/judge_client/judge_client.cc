...
void _update_solution_mysql(int solution_id, int result, int time, int memory,
    ...
    if(oi_mode){
        sprintf(
            sql,
            "UPDATE solution SET result=%d,time=%d,memory=%d,judgetime=NOW(),pass_rate=%f WHERE id=%d LIMIT 1%c",
            result, time, memory, pass_rate,solution_id, 0);
    }else{
        sprintf(
            sql,
            "UPDATE solution SET result=%d,time=%d,memory=%d,judgetime=NOW() WHERE id=%d LIMIT 1%c",
...
void _update_problem_mysql(int p_id) {
    ...
    sprintf(
        sql,
        "UPDATE `problem` SET `accepted`=(SELECT count(*) FROM `solution` WHERE `problem_id`=\'%d\' AND `result`=\'4\') WHERE `id`=\'%d\'",
        p_id, p_id);
    if (mysql_real_query(conn, sql, strlen(sql)))
        write_log(mysql_error(conn));
    sprintf(
        sql,
        "UPDATE `problem` SET `submit`=(SELECT count(*) FROM `solution` WHERE `problem_id`=\'%d\') WHERE `id`=\'%d\'",
        p_id, p_id);
    if (mysql_real_query(conn, sql, strlen(sql)))
        write_log(mysql_error(conn));
}
...
void _get_solution_info_mysql(int solution_id, int & p_id, char * user_id, int & lang, int &contest_id) {
    ...
    sprintf(
        sql,
        "SELECT problem_id, user_id, language, contest_id FROM solution where id=%d",
        solution_id);
    //printf("%s\n",sql);
    mysql_real_query(conn, sql, strlen(sql));
    res = mysql_store_result(conn);
...
void get_solution_info(int solution_id, int & p_id, char * user_id, int & lang, int &contest_id) {

    if(http_judge){
        _get_solution_info_http(solution_id,p_id,user_id,lang);
    }else{
        _get_solution_info_mysql(solution_id,p_id,user_id,lang, contest_id);
    }
}
...
map<string, int> get_problem_data_score(int problem_id){
    MYSQL_RES *res;
    MYSQL_ROW row;
    ...
    sprintf(
        sql,
        "SELECT file_name, score FROM  score where problem_id=%d order by (file_name)",
        problem_id);
    //printf("%s\n",sql);
    mysql_real_query(conn, sql, strlen(sql));
    res = mysql_store_result(conn);
...
void get_contest_info(int contest_id){
    ...
    sprintf(
        sql,
        "SELECT mode FROM contest where id=%d",
        contest_id);
    //printf("%s\n",sql);
    mysql_real_query(conn, sql, strlen(sql));
    res = mysql_store_result(conn);
...
int main(int argc, char** argv) {
    ...
    if(http_judge)
        system("ln -s ../cookie ./");
    get_solution_info(solution_id, p_id, user_id, lang, contest_id);
    if(contest_id != 0){

        get_contest_info(contest_id);
    }
    //get the limit
    ...
    int p_score = 0;
    if(oi_mode)
        mmap = get_problem_data_score(p_id);
    ...
    for (;(oi_mode|| ACflg == OJ_AC )&& (dirp = readdir(dp)) != NULL;) {
        ...
        if(DEBUG)
            printf("\nfile ---->%s score ------->%d\n", dirp->d_name, mmap[(string)dirp->d_name]);
        ...
        if (pidApp == 0) {
            ...
            //xin
            if(oi_mode){
                p_score = mmap[(string)dirp->d_name];
                if(p_score == 0)
                    p_score = 100;
                score_sum += (double)p_score;
            }
